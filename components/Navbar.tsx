...
export default function Navbar() {
  ...
  // Fix 5 — Removed "Buying Guides" — now Tech | Home & Kitchen | Blog
  const navLinks = [
    ...
    <nav style={{ backgroundColor: "#fff", borderBottom: "1px solid #E5E4E0", position: "sticky" as const, top: 0, zIndex: 100 }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 58 }}>
          ...
          {navLinks.map(link => {
            ...
            return (
              <Link key={link.href} href={link.href}
                style={{ fontSize: 13, fontWeight: 600, color: active ? "#0d9488" : "#374151", padding: "6px 14px", borderRadius: 8, textDecoration: "none", background: active ? "#E6F7F5" : "transparent" }}>
                {link.label}
              </Link>
            );
          })}
        ...
        <button className="hamburger" onClick={() => setMenuOpen(!menuOpen)}
          style={{ display: "none", background: "none", border: "none", cursor: "pointer", padding: 6, flexDirection: "column" as const, gap: 5 }} aria-label="Toggle menu">
          <span style={{ display: "block", width: 22, height: 2, background: "#1C1C1E", borderRadius: 2, transform: menuOpen ? "rotate(45deg) translate(5px, 5px)" : "none", transition: "all 0.2s" }} />
