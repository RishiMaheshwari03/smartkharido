...
function FilterPopover({ open, onClose, anchorRef, categories, hasGuides, hasReviews, activeCategory, activeType, sort, onCategory, onType, onSort }: {
  ...
  const tabs: { id: Tab; label: string; badge?: string }[] = [
    ...
    { id: "type",     label: "Type",     badge: activeType !== "All" ? (activeType === "Guide" ? "Guide" : "Review") : undefined },
    { id: "sort",     label: "Sort",     badge: sort === "oldest" ? "Oldest" : undefined },
  ];
  ...
  return (
    <div ref={ref} style={{
      ...
      zIndex: 300, width: 300, overflow: "hidden",
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #F3F4F6", padding: "6px 6px 0" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            {t.badge && (
              <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", background: "#0d9488", borderRadius: 100, padding: "1px 7px", maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{t.badge}</span>
            )}
          </button>
        ))}
      </div>
          ...
          <>
            ...
            <div style={{ maxHeight: 220, overflowY: "auto" as const }}>
              {filteredCats.length === 0 ? (
                <p style={{ fontSize: 12, color: "#9CA3AF", textAlign: "center" as const, padding: "16px 0" }}>No categories found</p>
              ) : filteredCats.map(cat => {
                const s = getCatStyle(cat);
                const active = activeCategory === cat;
                return (
                  <button key={cat} onClick={() => { onCategory(cat); onClose(); }}
                    style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 10, border: "none", cursor: "pointer", background: active ? s.bg : "transparent", textAlign: "left" as const, transition: "background 0.1s", marginBottom: 2 }}>
            ...
            {[
              ...
              return (
                <button key={t.v} onClick={() => { onType(t.v); onClose(); }}
                  ...
                  {active && <span style={{ fontSize: 13, color: "#0d9488", flexShrink: 0 }}>✓</span>}
                </button>
              );
            })}
          </div>
    ...
    <div style={{ backgroundColor: "#F7F6F3", minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", maxWidth: 580 }}>
            <div style={{ position: "relative" as const, flex: 1 }}>
              ...
              {search && (
                <button onClick={() => setSearch("")} style={{ position: "absolute" as const, right: 10, top: "50%", transform: "translateY(-50%)", background: "#E5E4E0", border: "none", borderRadius: "50%", width: 20, height: 20, fontSize: 10, cursor: "pointer", color: "#374151", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
              )}
            </div>
            ...
            <div style={{ position: "relative" as const, flexShrink: 0 }}>
              <button ref={filterBtnRef} onClick={() => setFilterOpen(v => !v)}
                style={{ display: "flex", alignItems: "center", gap: 7, padding: "13px 18px", borderRadius: 14, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 13, background: activeCount > 0 ? "#0d9488" : "#fff", color: activeCount > 0 ? "#fff" : "#374151", boxShadow: "0 4px 20px rgba(0,0,0,0.15)", transition: "all 0.15s", whiteSpace: "nowrap" as const }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const, marginTop: 14 }}>
              {activeCategory !== "All" && (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 600, background: "rgba(255,255,255,0.12)", color: "#fff", borderRadius: 100, padding: "5px 12px", border: "1px solid rgba(255,255,255,0.2)" }}>
                  📂 {activeCategory}
                  <button onClick={() => setActiveCategory("All")} style={{ background: "none", border: "none", cursor: "pointer", color: "#fff", fontSize: 11, padding: 0, opacity: 0.7, lineHeight: 1 }}>✕</button>
                </span>
              )}
              {activeType !== "All" && (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 600, background: "rgba(255,255,255,0.12)", color: "#fff", borderRadius: 100, padding: "5px 12px", border: "1px solid rgba(255,255,255,0.2)" }}>
                  {activeType === "Guide" ? "🗂 Guides" : "⭐ Reviews"}
                  <button onClick={() => setActiveType("All")} style={{ background: "none", border: "none", cursor: "pointer", color: "#fff", fontSize: 11, padding: 0, opacity: 0.7, lineHeight: 1 }}>✕</button>
                </span>
              )}
              {sort !== "newest" && (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 600, background: "rgba(255,255,255,0.12)", color: "#fff", borderRadius: 100, padding: "5px 12px", border: "1px solid rgba(255,255,255,0.2)" }}>
                  📅 Oldest first
                  <button onClick={() => setSort("newest")} style={{ background: "none", border: "none", cursor: "pointer", color: "#fff", fontSize: 11, padding: 0, opacity: 0.7, lineHeight: 1 }}>✕</button>
                </span>
              )}
            </div>
          )}
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "28px 20px 80px" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center" as const, padding: "64px 24px", background: "#fff", borderRadius: 20, border: "1px solid #E5E4E0" }}>
            ...
            <p style={{ fontSize: 14, color: "#9CA3AF", marginBottom: 20 }}>Try different keywords or clear your filters</p>
            <button onClick={() => { setSearch(""); setActiveCategory("All"); setActiveType("All"); setSort("newest"); }}
              style={{ background: "#0d9488", color: "#fff", border: "none", padding: "12px 24px", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Clear everything</button>
          </div>
        ) : (
      ...
      <style>{`
        ...
        @media (max-width: 520px) {
          /* On mobile popover goes full width */
          .filter-popover { right: auto !important; left: 0 !important; width: calc(100vw - 40px) !important; }
        }
      `}</style>
    </div>
  );
