...
function ProductCard({ product, index }: { product: Product; index: number }) {
  ...
  return (
    <div style={{ marginBottom: 48, borderRadius: 20, overflow: "hidden", border: "1px solid #E5E4E0", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
      <div style={{ background: "linear-gradient(90deg,#1C1C1E,#2a2a2d)", padding: "18px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" as const }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 17, fontWeight: 800, color: "#fff", letterSpacing: "-0.3px", lineHeight: 1.2 }}>{product.name}</div>
        ...
        <div style={{ textAlign: "right" as const }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 1, justifyContent: "flex-end", marginBottom: 4 }}>
            <span style={{ fontSize: 30, fontWeight: 900, color: "#0d9488", lineHeight: 1 }}>{product.overallRating}</span>
      ...
      <div style={{ background: "#fff", padding: "24px" }}>
        <div style={{ background: "#F0FDF9", border: "1px solid #b2ddd8", borderRadius: 12, padding: "13px 16px", marginBottom: 22, display: "flex", gap: 10, alignItems: "flex-start" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: product.image ? "1fr 1fr" : "1fr", gap: 18, marginBottom: 20, alignItems: "stretch" }}>
          {product.image && (
            <div style={{ borderRadius: 12, overflow: "hidden", border: "1px solid #E5E4E0", minHeight: 240 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <a href={product.amazon} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 7, background: "#FF9900", color: "#fff", padding: "13px 16px", borderRadius: 12, fontSize: 14, fontWeight: 800, textDecoration: "none" }}>🛒 Check on Amazon</a>
          <a href={product.flipkart} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 7, background: "#2874F0", color: "#fff", padding: "13px 16px", borderRadius: 12, fontSize: 14, fontWeight: 800, textDecoration: "none" }}>🛒 Check on Flipkart</a>
        </div>
      </div>
    </div>
  );
...
export default function ComparisonLayout({ post }: { post: any }) {
  ...
  return (
    <div style={{ backgroundColor: "#F7F6F3", minHeight: "100vh" }}>
      <div style={{ backgroundColor: "#fff", borderBottom: "1px solid #E5E4E0", padding: "48px 24px 32px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <h1 style={{ fontSize: "clamp(22px, 3.5vw, 36px)", fontWeight: 800, color: "#1C1C1E", lineHeight: 1.25, letterSpacing: "-0.8px", marginBottom: 14 }}>{post.title}</h1>
          <p style={{ fontSize: 16, color: "#6B7280", lineHeight: 1.7, marginBottom: 20, maxWidth: 680 }}>{post.excerpt}</p>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "40px 24px 80px" }}>
        {products.length > 0 && (
          <div style={{ marginBottom: 48 }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: "#1C1C1E", marginBottom: 16, letterSpacing: "-0.4px", display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ display: "inline-block", width: 4, height: 22, background: "#0d9488", borderRadius: 2 }} />
              Quick Comparison — All {products.length} Picks
            </h2>
            <div style={{ borderRadius: 14, overflow: "hidden", border: "1px solid #E5E4E0", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, background: "#fff" }}>
                <thead>
                  <tr style={{ background: "#1C1C1E" }}>
                    {["#", "Product", "Best For", "Rating", "Price", "Buy"].map(h => (
                      <th key={h} style={{ padding: "12px 16px", textAlign: "left" as const, fontWeight: 700, color: "#fff", fontSize: 11, textTransform: "uppercase" as const, letterSpacing: "0.5px" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {products.map((p, i) => (
                    <tr key={p.name} style={{ background: i % 2 === 0 ? "#fff" : "#FAFAF8" }} className="trow">
                      <td style={{ padding: "12px 16px", borderBottom: "1px solid #F3F4F6", color: "#9CA3AF", fontWeight: 700, fontSize: 12 }}>#{i + 1}</td>
                      <td style={{ padding: "12px 16px", borderBottom: "1px solid #F3F4F6" }}>
                        <a href={p.amazon} target="_blank" rel="noopener noreferrer" style={{ fontWeight: 700, color: "#1C1C1E", textDecoration: "none", borderBottom: "1.5px solid #0d948860" }}>{p.name}</a>
                      </td>
                      <td style={{ padding: "12px 16px", color: "#0d9488", fontWeight: 600, borderBottom: "1px solid #F3F4F6" }}>{p.best}</td>
                      <td style={{ padding: "12px 16px", borderBottom: "1px solid #F3F4F6" }}>
                        <span style={{ fontWeight: 800, color: "#0d9488", fontSize: 15 }}>{p.overallRating}</span>
                        <span style={{ color: "#9CA3AF", fontSize: 11 }}>/10</span>
                      </td>
                      <td style={{ padding: "12px 16px", color: "#374151", borderBottom: "1px solid #F3F4F6", fontWeight: 600 }}>{p.price}</td>
                      <td style={{ padding: "12px 16px", borderBottom: "1px solid #F3F4F6" }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          <a href={p.amazon} target="_blank" rel="noopener noreferrer" style={{ background: "#FF9900", color: "#fff", padding: "5px 12px", borderRadius: 7, fontSize: 11, fontWeight: 700, textDecoration: "none" }}>Amazon</a>
                          <a href={p.flipkart} target="_blank" rel="noopener noreferrer" style={{ background: "#2874F0", color: "#fff", padding: "5px 12px", borderRadius: 7, fontSize: 11, fontWeight: 700, textDecoration: "none" }}>Flipkart</a>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div id="final-verdict" style={{ marginBottom: 48 }}>
          <h2 style={{ fontSize: 20, fontWeight: 800, color: "#1C1C1E", marginBottom: 4, letterSpacing: "-0.4px", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ display: "inline-block", width: 4, height: 20, background: "#0d9488", borderRadius: 2 }} />Final Verdict
          </h2>
          <p style={{ fontSize: 13, color: "#9CA3AF", marginBottom: 16 }}>Click any product name to check the current price on Amazon India.</p>
          <div style={{ borderRadius: 14, overflow: "hidden", border: "1px solid #E5E4E0", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14, background: "#fff" }}>
              <thead>
                <tr style={{ background: "#1C1C1E" }}>
                  <th style={{ padding: "12px 20px", textAlign: "left" as const, color: "#fff", fontSize: 11, fontWeight: 700, textTransform: "uppercase" as const, width: "40%" }}>Your Priority</th>
                  <th style={{ padding: "12px 20px", textAlign: "left" as const, color: "#fff", fontSize: 11, fontWeight: 700, textTransform: "uppercase" as const }}>Best Pick</th>
                </tr>
              </thead>
              <tbody>
                {products.map((p, i) => (
                  <tr key={i} className="trow" style={{ background: i % 2 === 0 ? "#fff" : "#FAFAF8" }}>
                    <td style={{ padding: "14px 20px", color: "#374151", borderBottom: "1px solid #F3F4F6", fontWeight: 500 }}>{p.best}</td>
                    <td style={{ padding: "14px 20px", borderBottom: "1px solid #F3F4F6" }}>
                      <a href={p.amazon} target="_blank" rel="noopener noreferrer" style={{ color: "#0d9488", fontWeight: 700, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 5 }}>
                        {p.name} <span style={{ fontSize: 12, opacity: 0.7 }}>→</span>
                      </a>
                    </td>
                  </tr>
                ))}
        ...
        <div style={{ padding: "36px 32px", background: "linear-gradient(135deg, #0d9488 0%, #0f766e 100%)", borderRadius: 20, textAlign: "center" as const, color: "#fff" }}>
          <p style={{ fontSize: 22, fontWeight: 800, marginBottom: 8 }}>Found this guide helpful?</p>
          <p style={{ fontSize: 15, opacity: 0.85, marginBottom: 24 }}>Browse more honest buying guides for Indian buyers</p>
      ...
      <style>{`.trow:hover td { background: #F0FDF9 !important; transition: background 0.15s; }`}</style>
    </div>
  );
}