...
function ReviewScoreBar({ label, score }: { label: string; score: number }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      {/* UX 4 — label and score on separate lines if needed, never truncate */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5, gap: 8 }}>
        <span style={{ fontSize: 13, color: "#374151", fontWeight: 600, minWidth: 0, wordBreak: "break-word" as const }}>{label}</span>
        <span style={{ fontSize: 13, fontWeight: 800, color, flexShrink: 0 }}>{score}/10</span>
      </div>
    ...
    <div style={{ backgroundColor: "#F7F6F3", minHeight: "100vh" }}>
      ...
      <div style={{ backgroundColor: "#fff", borderBottom: "1px solid #E5E4E0", padding: "40px 20px 0" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          {/* UX 7 — date always visible on its own row */}
          <div style={{ display: "flex", flexWrap: "wrap" as const, alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 12, color: "#bbb", whiteSpace: "nowrap" as const }}>
          ...
          {/* UX 5 — TOC: scrollable horizontally on mobile instead of wrapping to 3 lines */}
          <div style={{ display: "flex", gap: 6, marginTop: 20, overflowX: "auto", paddingBottom: 2, WebkitOverflowScrolling: "touch" as any }}>
            {(rp.sections || []).map(s => (
              <a key={s.id} href={`#${s.id}`} style={{ fontSize: 12, fontWeight: 600, color: "#0d9488", background: "#E6F7F5", padding: "6px 12px", borderRadius: 100, textDecoration: "none", whiteSpace: "nowrap" as const, flexShrink: 0 }}>{s.icon} {s.title}</a>
            ))}
            <a href="#verdict" style={{ fontSize: 12, fontWeight: 600, color: "#fff", background: "#0d9488", padding: "6px 12px", borderRadius: 100, textDecoration: "none", whiteSpace: "nowrap" as const, flexShrink: 0 }}>⚖️ Verdict ↓</a>
          </div>
        </div>
      </div>
